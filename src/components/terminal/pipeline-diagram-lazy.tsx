 ...
 *
 * The skeleton mirrors the diagram's box (monospace lines with the
 ...
 * render prop doesn't receive the `lines` prop passed to the dynamic
 * component, so it can't size itself per chapter — 212px is sized for the
 * tallest current "terminal" chapter (case-study.ts's "pipeline", 9 lines);
 * the shorter "workflow" chapter (8 lines) ends up ~1 line shorter than its
 * skeleton, a small harmless shrink instead of the growth that caused a real
 * CLS hit on /link-charts. Was 232px while the box also drew a title line.
 */
export const PipelineDiagramLazy = dynamic(
  () => import("./pipeline-diagram").then((mod) => mod.PipelineDiagram),
  {
    ...
    loading: () => (
      <div
        className="h-[212px] animate-pulse rounded-lg border border-border bg-surface-deep"
        aria-hidden="true"
      />
    ),
  },
